...
const BranchSelectBox = ({ }) => {
  ...
  const { branches, selectedBranch, setSelectedBranch, revalidate } = useBranch();
  ...
  return (
    <>
      <div className={`collapse collapse-arrow md:w-1/4 border-neutral border btn-outline btn-neutral w-full transition-all ${isOpen && "bg-neutral-content !text-black md:!w-1/2"}`}>
        ...
        <div className={`collapse-title ${(selectedBranch?.id && !isOpen) && "text-primary"}`}>{selectedBranch?.name || "Branch"}</div>
        <div className="collapse-content flex flex-col gap-4">
          ...
          {branches?.map(element => <Branch key={element?.id} isSelected={(selectedBranch?.id === element?.id)} branch={element} />)}
        </div>
      </div>
