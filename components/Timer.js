...
const Timer = () => {
    ...
    const { selectedTask, currentTimeEntry, setCurrentTimeEntry, setIsTimerRunning } = useBranch();
        ...
        <div className='hero my-8'>
            <div className='hero-content flex flex-col gap-8 md:gap-16'>
                ...
                <div className="flex gap-8">
                    ...
                    <div onClick={isRunning ? pause : start} className={`${!selectedTask?.id && "btn-disabled cursor-not-allowed"} btn-lg btn btn-primary btn-outline p-4`}>
                        {
                            isRunning
                                ?
                    ...
                    <div onClick={stop} className={`${!selectedTask?.id && "btn-disabled"} ${isRunning ? "bg-red-600 border-red-600" : "btn-outline"} btn-lg btn btn-primary p-4`}><svg className="w-full h-full" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <rect x="4" y="4" width="16" height="16" rx="2" className="stroke-current fill-current" stroke="#161212" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></rect> </g></svg></div>
                </div>
            </div>
        </div>
    )
